...
you need to assess the overall happiness rating of the group. If <= 5, return 'Get Out Now!'. Else return 'Nice Work
Champ!'.

Happiness rating will be total score / number of people in the room.

Note that your boss is in the room (boss), their score is worth double it's face value (but they are still just one
person!). """
import codewars_test as test


def outed(meet, boss):
    """Вычисляем средний рейтинг отношения сотрудников к вам"""
    c = (sum(meet.values()) + meet[boss]) / len(meet.values())
    if c > 5:
...
print(outed(
    {'tim': 0, 'jim': 2, 'randy': 0, 'sandy': 7, 'andy': 0, 'katie': 5, 'laura': 1, 'saajid': 2, 'alex': 3, 'john': 2,
     'mr': 0}, 'laura'))
...
test.assert_equals(outed(
    {'tim': 1, 'jim': 3, 'randy': 9, 'sandy': 6, 'andy': 7, 'katie': 6, 'laura': 9, 'saajid': 9, 'alex': 9, 'john': 9,
     'mr': 8}, 'katie'), 'Nice Work Champ!')
test.assert_equals(outed(
    {'tim': 2, 'jim': 4, 'randy': 0, 'sandy': 5, 'andy': 8, 'katie': 6, 'laura': 2, 'saajid': 2, 'alex': 3, 'john': 2,
